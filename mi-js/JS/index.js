...
//分类列表交互功能
function handleCate(){
	var aCateItem = document.querySelectorAll('.home .banner .banner-cart .banner-cart-item');
	var oCateContent = document.querySelector('.home .banner .banner-cart-content');
	var oCateBox = document.querySelector('.home .banner .cart-box');
	for(var i=0;i<aCateItem.length;i++){
		aCateItem[i].index = i;
		aCateItem[i].onmouseenter = function(){
			for(var j=0;j<aCateItem.length;j++){
				aCateItem[j].className = "banner-cart-item";
			}
			this.className = "banner-cart-item active";
...
function handleTimeDown(){
	...
	var endDate = new Date('2019/9/6 21:00:00');
...
function handleFlash(){
	...
	var oProductBox = document.querySelector('.flash .bd .bd-right');
...
function handleElecPart(){
	...
	var oTabContent = document.querySelector('.elec .bd-list .product-list');
	loadedata(0);
	...
	function loadedata(index){
		...
		for(var i=0;i<data.length-1;i++){
			...
			html +=		'</p>';
			if(data[i].new){
				html +=		'<span class="'+data[i].new.name+'">'+data[i].new.content+'</span>';
			}
			if(data[i].comment){
